...
import com.tw.networking.data.BookDoc
...
class OpenLibraryService @Inject constructor(
     ...
     */
    suspend fun searchBooks(query: String): RequestResult<SearchResponse> =
        RequestResult.fromResponse { api.searchBooks(query) }
     ...
     */
    suspend fun getBookDetails(bookId: String): RequestResult<Book> =
        RequestResult.fromResponse { api.getBookDetails(bookId).toBook() }
}
